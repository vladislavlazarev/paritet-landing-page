...
export default async function LocaleServicesPage(
  ...
  const breadcrumbs = breadcrumbJsonLd([
    { name: dict.servicesPage.breadcrumbHome, url: localizedAbsoluteUrl(locale, "/") },
    { name: dict.servicesPage.breadcrumbServices, url: localizedAbsoluteUrl(locale, "/services") },
  ]);
      ...
      <main className="flex-1">
        ...
        <section className="bg-surface-soft">
          <div className="container-page py-14 sm:py-20 lg:py-28">
            <p className="mb-10 max-w-3xl text-[14px] leading-relaxed text-muted-fg border-l-2 border-accent-coral/40 pl-4">
              ...
              {SERVICE_GROUPS.map((g) => (
                ...
                >
                  ...
                  <div className="px-5 sm:px-10 lg:px-14 py-7 sm:py-11 lg:py-14">
                    ...
                    >
                      <h2 lang="ru" className="font-heading text-[22px] sm:text-[30px] lg:text-[40px] leading-[1.05] tracking-[-0.02em] text-brand transition-colors group-hover/title:text-[color:var(--accent)]">
                        {g.title}
                      </h2>
                      <span
                        aria-hidden
                    ...
                    <ul className="mt-7 sm:mt-8 flex flex-wrap gap-2.5 sm:gap-3">
                      {g.items.map((item, j) => {
                        if (!item.slug) {
                          return (
                            <li key={j}>
                              <span lang="ru" className="inline-flex items-center rounded-full ring-1 ring-hairline bg-surface-soft px-4 py-2.5 text-[14px] sm:text-[15px] leading-none text-muted-fg">
                                {item.label}
                              </span>
                            </li>
                          );
